...
// -- 11z. The gate's reading reaches the reviewer (SS4g) -------
// rule_audit.py compared a stated period against the held text for 151 of 419
// rules since v159 and printed it to a terminal. Nobody verifying a rule was
// looking at a terminal -- SS2j/SS3n, a value computed correctly that reaches
// no screen.
describe('rule verification evidence');
{
  const G = app.LG_GATE || {};
  ok('the gate result is embedded', !!(G.rules && Object.keys(G.rules).length),
     'LG_GATE.rules is empty');

  const RULES = app.lgAllRules();
  // The blob is generated by rule_audit.py and re-embedded on every run. A
  // generated file that goes stale is SS3t's amendments.py and SS4b's corpus
  // blobs -- so the corpus and the gate must not drift apart.
  const ungated = RULES.filter(r => !G.rules[r.id]).map(r => r.id);
  ok('every rule in the corpus has a gate verdict', ungated.length === 0,
     ungated.length + ' without one, e.g. ' + ungated.slice(0, 3).join(', ') +
     ' -- run: python tools/rule_audit.py');
  const orphan = Object.keys(G.rules).filter(id => !RULES.some(r => r.id === id));
  ok('and the gate holds no verdict for a rule that is gone', orphan.length === 0,
     orphan.slice(0, 3).join(', '));

  const codes = Object.keys(G.codes || {});
  const unnamed = Object.keys(G.rules)
    .map(id => G.rules[id].v).filter(v => codes.indexOf(v) < 0);
  ok('every verdict a rule carries has a description',
     unnamed.length === 0, Array.from(new Set(unnamed)).join(', '));

  // lgAllRules carried the title alone, so the modal showed what an obligation
  // is CALLED and nothing it ASSERTS.
  // Counted per FIELD, not as an "any of these" sweep. The first version read
  // `timelineText || trigger || appliesToText` over a threshold of 250, so
  // blanking timelineText alone still passed -- the mutation runner reported it
  // MISSED. A sweep wide enough to survive one field going dark is not testing
  // that field.
  const nWith = (f) => RULES.filter(r => r[f]).length;
  ok('the period a rule states reaches the reviewer', nWith('timelineText') > 230,
     'timelineText on ' + nWith('timelineText') + ' of ' + RULES.length);
  ok('and what it applies to', nWith('appliesToText') > 200,
     'appliesToText on ' + nWith('appliesToText'));
  ok('and a hand-authored rule carries the words it was read from',
     nWith('quote') > 80, nWith('quote') + ' carry a quote');

  // ── what actually renders ─────────────────────────────────
  const agreed = RULES.find(r => (G.rules[r.id] || {}).v === 'ok' && G.rules[r.id].x);
  ok('there is a rule the gate confirmed', !!agreed, 'none found');
  const gb = agreed ? app.govGateBlock(agreed) : '';

  // THE ASSEMBLY, not the leaf. Every assertion here called govGateBlock
  // DIRECTLY, so removing it from the modal changed nothing they could see and
  // the mutation went MISSED -- SS3n's shape for the third time in this
  // project. govEvidenceBlock is what the modal actually renders.
  const assembled = agreed ? app.govEvidenceBlock(agreed) : '';
  ok('the gate verdict reaches the modal, not just the function',
     assembled.indexOf('release gate') >= 0, assembled.slice(0, 140));
  ok('and so does what the rule itself claims',
     assembled.indexOf('What this rule claims') >= 0, assembled.slice(0, 140));
  ok('the verdict reaches the screen', gb.indexOf('release gate') >= 0, gb.slice(0, 80));
  ok('with the provision\'s own words beneath it',
     gb.indexOf(G.rules[agreed.id].x.slice(0, 40)) >= 0, 'the excerpt did not render');

  // THE HONESTY CONTROL. SS3v measured 4 of 23 agreements falling outside the
  // sub-clause the rule cites, and WITHDREW the narrowing rather than ship one
  // wrong half the time. An agreement that does not carry that limit reads as
  // a sign-off, which is the one thing this screen must never do.
  ok('an agreement carries the limit SS3v measured',
     gb.indexOf('not against the sub-clause') >= 0 ||
     gb.indexOf(String(G.limit).slice(0, 40)) >= 0, gb.slice(-320));
  ok('and says it is the reading, not the decision',
     /reading, not the decision/i.test(gb), gb.slice(-200));
  ok('and never calls the rule verified or current',
     !/\bis (verified|current)\b/i.test(gb), gb.slice(-200));

  // Total, so a corpus that stops carrying quotes FAILS BY NAME rather than
  // throwing on undefined. SS4d recorded exactly this one release ago: a
  // mutation caught by a crash tells you nothing about the assertion meant to
  // catch it.
  const quoted = RULES.find(r => r.quote) || null;
  ok('a rule still carries a verbatim quote', !!quoted,
     'no rule in any corpus carries one');
  const cb = quoted ? app.govClaimBlock(quoted) : '';
  ok('the verbatim quote reaches the reviewer',
  ...
  check('and a rule the gate never saw renders nothing',
